...
export const getEmployeeById = async (req, res) => {
  ...
  try {
    let employee = null;
    const employeeData = await EmployeeMod.findById(userId).populate('superAdminId');
    if (employeeData) {
      const superAdmin = await AdminModel.findById(employeeData.superAdminId).populate('employees');
      if (superAdmin) {
        const employeeId = req.params.employeeId;
        employee = superAdmin.employees.find(emp => emp._id.toString() === employeeId);
      }
    } else {
  ...
  try {
    ...
    if (employee && employee.superAdminId) {
      ...
      if (superAdmin) {
        ...
        return res.status(201).json({ message: "New customer has been added successfully." });
      }
    } else {
...
export const getAllCustomers = async (req, res) => {
  ...
  try {
    ...
    if (employee) {
      const superAdmin = await AdminModel.findById(employee.superAdminId);
      if (superAdmin) {
        customers = await CustomerModel.find({ _id: { $in: superAdmin.customers } });
      }
    }
...
export const getCustomerById = async (req, res) => {
  ...
  try {
    ...
    const employee = await EmployeeMod.findById(userId).populate('superAdminId');
    if (employee) {
      const superAdmin = await AdminModel.findById(employee.superAdminId).populate('customers');
      if (superAdmin) {
        const customerId = req.params.customerId;
        customer = superAdmin.customers.find(cust => cust._id.toString() === customerId);
      }
    } else {
      const superAdmin = await AdminModel.findById(userId).populate('customers');
      if (superAdmin) {
        const customerId = req.params.customerId;
        customer = superAdmin.customers.find(cust => cust._id.toString() === customerId);
      }
...
export const getCustomerInvoice = async (req, res) => {
  ...
  try {
    const customer = await CustomerModel.findById(customerId).populate("invoice");
    if (!customer) {
      return res.status(404).json({ message: "Customer not found." });
    }
...
export const editCustomer = async (req, res) => {
  ...
  try {
    ...
    const customer = await CustomerModel.findByIdAndUpdate(customerId, updatedFields, {
      new: true,
    });
    if (!customer) {
      return res.status(404).json({ message: "Customer not found." });
...
export const editStaff = async (req, res) => {
  ...
  try {
    const employeeId = req.params.employeeId;
    const updatedFields = req.body;
    const employee = await  EmployeeMod.findByIdAndUpdate(employeeId, updatedFields, {
      new: true,
    });
    if (!employee) {
      return res.status(404).json({ message: "Employee not found." });
...
export const deleteStaff = async (req, res) => {
  ...
  try {
    ...
    const staffIndex = admin.employees.findIndex(emp => emp._id.toString() === employeeId);
    if (staffIndex === -1) {
...
export const createCreditCase = async (req, res) => {
  ...
  try {
    const { customerId } = req.params;
    const { product: productId, quantity, dueDate, issuedDate, status } = req.body;
    ...
    customer.creditCase.push(newCreditCase);
    await customer.save();
    product.quantity -= quantity;
    await product.save();
    return res.status(201).json({ message: 'Credit case created successfully' });
  } catch (error) {
    console.error('Error:', error);
    return res.status(500).json({ message: 'Internal Server Error' });
  }
...
export const getCustomerCredit = async (req, res) => {
  ...
  try {
    ...
    const creditCases = customer.creditCase.map(caseItem => {
      if (caseItem.dueDate < today) {
        caseItem.status = "Overdue";
      }
      return caseItem;
    });

    return res.status(200).json({ creditCases }); 
  } catch (error) {
    console.error("Error:", error);
    return res.status(500).json({ message: "Internal Server Error" });
  }
...
export const settleCreditCase = async (req, res) => {
  const userId = req.userAuth;
  if (!userId) {
    return res.status(401).json({ error: 'Unauthorized' });
  }
  try {
    const creditCaseId = req.params.creditCaseId;
    const creditCase = await CustomerModel.findOneAndUpdate(
      { 'creditCase._id': creditCaseId },
      { $set: { 'creditCase.$.status': 'Settled', 'creditCase.$.settledDate': new Date() } },
      { new: true }
    );
    if (!creditCase) {
      return res.status(404).json({ message: 'Credit case not found.' });
    }
    return res.status(200).json({ message: 'Credit case has been settled successfully.' });
  } catch (error) {
    console.error('Error:', error);
    return res.status(500).json({ message: 'Internal Server Error' });
  }
  ...
  export const updateUser = async (req, res) => {
    try {
      const userId = req.userAuth;
      const updatedFields = req.body;
      let user = await EmployeeMod.findByIdAndUpdate(
        userId,
         updatedFields,
        { new: true, }
      );
